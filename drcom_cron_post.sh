...
then
    ...
    curl -s "https://api.telegram.org/bot${tg_token}/sendMessage?chat_id=${tg_id}" --data-binary "&text=【OpenWrt联网状态】当前路由器未联网，已执行登录。当前时间 ""`date`" &
    logger "【Dr.COM网页认证】已通过 Telegram Bot 发送当前时间"
else
    #已经登录
    logger "【Dr.COM网页认证】上网登录窗之前已登录"
    curl -s "https://api.telegram.org/bot${tg_token}/sendMessage?chat_id=${tg_id}" --data-binary "&text=【OpenWrt联网状态】当前路由器之前已登录，无需执行登录。当前时间 ""`date`" &
    logger "【Dr.COM网页认证】已通过 Telegram Bot 发送当前时间"
fi
